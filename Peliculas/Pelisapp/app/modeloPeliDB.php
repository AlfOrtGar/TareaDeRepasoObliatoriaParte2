...
class modeloPeliDB {
     ...
     private static $modifica_peli = "Update peliculas set nombre = ?, director = ?, genero = ?, video = ? where codigo_pelicula = ?";
     private static $borra_peli = "Delete from peliculas where codigo_pelicula = ?";
     private static $nueva_peli = "Insert into peliculas (codigo_pelicula,nombre,director,genero,video) Values (?,?,?,?,?)";
     private static $nuevo_voto = "Update peliculas set total_votos = ?, votos = ? where codigo_pelicula = ?";
...
public static function Nuevo($titulo,$director,$genero,$video){
    ...
    $stmt->bindValue(5,$video);
    $stmt->execute();
}

public static function Actualizar($id,$titulo,$director,$genero,$video){
...
public static function Votar($codigo,$voto){

    $stmt=self::$dbh->prepare("Select total_votos from peliculas where codigo_pelicula = ?");
    $stmt->bindValue(1,$codigo);
    $stmt->execute();
    ...
    $stmt=self::$dbh->prepare(self::$nuevo_voto);
